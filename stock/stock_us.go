...
const (
	// 东方财富美股列表 API
	emUSStockListURL = "https://push2.eastmoney.com/api/qt/clist/get"
	// 东方财富美股K线 API
	emUSKlineURL = "https://push2his.eastmoney.com/api/qt/stock/kline/get"
)

// StockUsSpotEm 获取美股实时行情（东方财富数据源）
//
// 返回:
//   - []USStockQuote: 美股行情列表
//   - error: 错误信息
...
// 示例:
//
//	quotes, err := stock.StockUsSpotEm()
//	if err != nil {
//	    log.Fatal(err)
//	}
//	for _, q := range quotes[:10] {
//	    fmt.Printf("%s(%s): %.2f %.2f%%\n", q.NameCN, q.Code, q.Price, q.ChangePct)
//	}
func StockUsSpotEm() ([]USStockQuote, error) {
	...
	resp, err := utils.GetWithHeaders(emUSStockListURL, params, headers)
	if err != nil {
		return nil, fmt.Errorf("获取美股行情失败: %w", err)
	}
	...
	if !result.Exists() {
		return nil, fmt.Errorf("解析美股行情数据失败: 未找到 %s", dataPath)
	}

	quotes := make([]USStockQuote, 0, result.Get("#").Int())
	now := time.Now()

	result.ForEach(func(_, value gjson.Result) bool {
		quote := USStockQuote{
			Code:      value.Get("f12").String(),
			Name:      value.Get("f12").String(), // 美股代码即为名称
			NameCN:    value.Get("f14").String(),
...
// StockUsDailyEm 获取美股历史K线（东方财富数据源）
//
// 参数:
//   - code: 美股代码，如 "AAPL"
...
// 示例:
//
//	klines, err := stock.StockUsDailyEm("AAPL", "20240101", "20240131", "qfq")
//	if err != nil {
//	    log.Fatal(err)
//	}
...
//	}
func StockUsDailyEm(code, startDate, endDate, adjust string) ([]StockKLine, error) {
	if code == "" {
		return nil, fmt.Errorf("股票代码不能为空")
	}

	// 美股代码转换 (需要先查询获取 secid)
	secid, err := getUSSecID(code)
	if err != nil {
		return nil, fmt.Errorf("获取美股代码失败: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("获取美股K线失败: %w", err)
	}
	...
	result := gjson.Get(text, dataPath)
	if !result.Exists() {
		return nil, fmt.Errorf("解析美股K线数据失败: 未找到 %s", dataPath)
	}
...
// getUSSecID 根据美股代码获取东方财富 secid
func getUSSecID(code string) (string, error) {
	// 常见美股市场代码映射
	// 105: 纳斯达克, 106: 纽交所, 107: 美国交易所
	markets := []string{"105", "106", "107"}
	...
	for _, market := range markets {
		...
		text := resp.String()
		if gjson.Get(text, "data.code").Exists() {
			return secid, nil
		}
	}
