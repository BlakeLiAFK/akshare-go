...
// StockZhBSpotSina 新浪B股实时行情
func StockZhBSpotSina() (dataframe.DataFrame, error) {
	countURL := ZhSinaBStockCountURL
	...
	for page := 1; page <= totalPages; page++ {
		...
		resp, err := utils.Get(ZhSinaBStockURL, payload)
		if err != nil {
			continue
		}
...
// StockZhBHistSina 新浪B股历史数据
func StockZhBHistSina(symbol, period, adjust string) (dataframe.DataFrame, error) {
	if symbol == "" {
	...
	default:
		url = fmt.Sprintf(ZhSinaBStockHistURL, fullSymbol)
	}
