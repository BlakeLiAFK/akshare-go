...
// FundLofSpotEm 获取LOF实时行情数据
// https://quote.eastmoney.com/center/gridlist.html#fund_lof
func FundLofSpotEm() ([]map[string]interface{}, error) {
	url := "https://push2.eastmoney.com/api/qt/clist/get"
	params := map[string]string{
		"pn":     "1",
		"pz":     "20000",
		"fs":     "b:MK0404,b:MK0405,b:MK0406",
...
// FundLofHistEm 获取LOF历史行情数据
// 参数:
//
//	symbol: LOF代码，如 "166009"
//	period: 周期 "daily"(日), "weekly"(周), "monthly"(月)
//	startDate: 开始日期 "19700101"
//	endDate: 结束日期 "20500101"
//	adjust: 复权类型 ""(不复权), "qfq"(前复权), "hfq"(后复权)
func FundLofHistEm(symbol, period, startDate, endDate, adjust string) ([]map[string]interface{}, error) {
	if symbol == "" {
		symbol = "166009"
	}
	if period == "" {
...
// FundLofHistMinEm 获取LOF分钟级别行情数据
// 参数:
//
//	symbol: LOF代码
...
//	adjust: 复权类型 ""(不复权), "qfq"(前复权), "hfq"(后复权)
func FundLofHistMinEm(symbol, startDate, endDate, period, adjust string) ([]map[string]interface{}, error) {
	if symbol == "" {
		symbol = "166009"
	}
	if period == "" {
