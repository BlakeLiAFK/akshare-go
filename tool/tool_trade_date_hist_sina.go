...
const (
	// 新浪交易日历 API
	sinaTradeDateURL = "https://finance.sina.com.cn/realstock/company/klc_td_sh.txt"

	// JS 解码函数 (来自 akshare Python 版本)
	hkJsDecode = `
function d(t) {
...
// ToolTradeDateHistSina 新浪财经-交易日历-历史数据
//
...
// 示例:
//
//	dates, err := tool.ToolTradeDateHistSina()
//	if err != nil {
//	    log.Fatal(err)
//	}
//	for _, d := range dates {
//	    fmt.Println(d.Date.Format("2006-01-02"))
//	}
func ToolTradeDateHistSina() ([]TradeDate, error) {
	// 获取数据
	resp, err := utils.Get(sinaTradeDateURL, nil)
	if err != nil {
	...
	// 提取加密数据
	text := resp.String()
	// 格式: var datelist="...";var KLC_TD_SH=d
	// 需要提取 datelist 的值
	startIdx := strings.Index(text, "var datelist=\"")
	if startIdx == -1 {
		return nil, fmt.Errorf("解析数据失败: 未找到 datelist")
	}
	startIdx += len("var datelist=\"")

	endIdx := strings.Index(text[startIdx:], "\"")
	if endIdx == -1 {
		return nil, fmt.Errorf("解析数据失败: 未找到 datelist 结束标记")
	}
	encodedData := text[startIdx : startIdx+endIdx]

	// 创建 JS 运行时
	vm := goja.New()
	_, err = vm.RunString(hkJsDecode)
	if err != nil {
		return nil, fmt.Errorf("初始化JS解码器失败: %w", err)
	}

	// 调用解码函数
	decodeFunc, ok := goja.AssertFunction(vm.Get("d"))
	if !ok {
		return nil, fmt.Errorf("获取解码函数失败")
	}

	// 执行解码
	result, err := decodeFunc(goja.Undefined(), vm.ToValue(encodedData))
	if err != nil {
		return nil, fmt.Errorf("执行解码函数失败: %w", err)
	}

	// 解析结果
	resultStr := result.String()
	if resultStr == "" || resultStr == "undefined" {
		return nil, fmt.Errorf("解码结果为空")
	}

	// 将 JS Date 对象数组转换为 Go 时间
	// JS 返回的是 Date 对象数组，格式类似: [Date, Date, ...]
	dates := make([]TradeDate, 0)

	// 使用 goja 解析数组
	export := vm.Get("JSON").ToObject(vm)
	callable, ok := goja.AssertFunction(export.Get("stringify"))
	if ok {
		jsonResult, err := callable(goja.Undefined(), result)
		if err == nil {
			// 使用 goja 迭代数组
			array := result.Export()
			if arr, ok := array.([]interface{}); ok {
				_ = jsonResult // 避免未使用变量警告
				for _, item := range arr {
					if dateVal, ok := item.(time.Time); ok {
						dates = append(dates, TradeDate{Date: dateVal})
					} else if timestamp, ok := item.(int64); ok {
						dates = append(dates, TradeDate{Date: time.Unix(timestamp/1000, 0)})
					} else if dateStr, ok := item.(string); ok {
						parsedTime, err := time.Parse(time.RFC3339, dateStr)
						if err == nil {
							dates = append(dates, TradeDate{Date: parsedTime})
						}
					}
				}
			}
	...
	// 如果上面的方法失败，尝试直接迭代 goja 值
	if len(dates) == 0 {
		if obj := result.Export(); obj != nil {
			// 尝试从字符串中解析日期
			// JS 可能返回类似: 1992-05-04T00:00:00.000Z
			...
			for _, line := range lines {
				...
				for _, format := range formats {
					if t, err := time.Parse(format, line); err == nil {
						dates = append(dates, TradeDate{Date: t})
						break
					}
				}
			}
	...
	// 补充缺失的日期 (1992-05-04)
	missingDate := time.Date(1992, 5, 4, 0, 0, 0, 0, time.UTC)
	found := false
	for _, d := range dates {
		if d.Date.Equal(missingDate) {
			found = true
			break
		}
	}
	if !found {
		dates = append(dates, TradeDate{Date: missingDate})
	}
